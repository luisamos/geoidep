...
def catalogo_listar():
  catalogos = (
    Categoria.query.filter_by(id_padre=PERSONAL_CATALOGO_PADRE_ID)
...
def _obtener_datos_catalogo(payload: dict[str, object]) -> tuple[dict[str, object], tuple[dict, int] | None]:
...
def catalogo_crear():
  ...
  datos, error = _obtener_datos_catalogo(payload)
  if error:
    return error
  ...
  registro = Categoria(
    ...
    id_padre=PERSONAL_CATALOGO_PADRE_ID,
...
def catalogo_actualizar(registro_id: int):
  ...
  if not registro or registro.id_padre != PERSONAL_CATALOGO_PADRE_ID:
...
@bp.route('/personas/<int:persona_id>', methods=['PUT'])
@jwt_required()
def personas_actualizar(persona_id: int):
...
@bp.route('/personas/<int:persona_id>', methods=['DELETE'])
@jwt_required()
def personas_eliminar(persona_id: int):
  persona = Persona.query.get(persona_id)
  if not persona:
    return jsonify({'status': 'error', 'message': 'Persona no encontrada.'}), 404
