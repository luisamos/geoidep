...
def _enviar_correo_confirmacion(usuario: Usuario) -> bool:
    ...
    try:
        mail.send(mensaje)
    except Exception as exc:  # pragma: no cover - dependiente de infraestructura
...
def _enviar_correo_recuperacion(usuario: Usuario) -> bool:
    if not usuario.email:
        current_app.logger.warning(
    ...
    try:
        mail.send(mensaje)
    except Exception:  # pragma: no cover - dependiente de infraestructura
...
def ingreso():
    ...
    if form.validate_on_submit():
        ...
        if not usuario or not usuario.check_password(form.password.data):
            ...
            session['access_token'] = create_access_token(identity=usuario.email)
...
@bp.route('/gestion/recuperar', methods=['GET', 'POST'])
def recuperar_contrasena():
    ...
    if form.validate_on_submit():
        ...
        if usuario and usuario.estado:
            usuario.generar_token_recuperacion()
            db.session.commit()
            if _enviar_correo_recuperacion(usuario):
...
@bp.route('/gestion/restablecer/<token>', methods=['GET', 'POST'])
...
@bp.route('/gestion/usuarios/guardar', methods=['POST'])
def guardar_usuario():
    if 'usuario' not in session:
        return redirect(url_for('gestion.ingreso'))
    ...
    if form.validate_on_submit():
        ...
        if form.id.data:
            ...
            if existe:
                ...
                return redirect(url_for('gestion.usuarios', editar=usuario.id))
        else:
            if Usuario.query.filter(func.lower(Usuario.email) == email).first():
...
@bp.route('/gestion/usuarios/<int:usuario_id>/eliminar', methods=['POST'])
