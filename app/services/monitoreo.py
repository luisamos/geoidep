...
from app.services.http import HEADERS_HTML, HEADERS_OGC
from app.services.ogc import (
    ID_TIPO_ARCGIS_KML,
    ID_TIPO_ARCGIS_MAPSERVER,
    ID_TIPO_WFS,
    ID_TIPO_WMS,
    TIPOS_ARCGIS,
    build_getcapabilities_url,
)
...
def build_service_url(servicio: ServicioGeografico) -> str:
    if servicio.id_tipo == ID_TIPO_WFS:
        return build_getcapabilities_url(servicio.direccion_web, "WFS")
    if servicio.id_tipo == ID_TIPO_WMS:
...
async def fetch_with_retries(
    ...
    request_headers = headers or HEADERS_OGC
...
async def procesar_servicio(
    ...
    if servicio.id_tipo in TIPOS_ARCGIS:
