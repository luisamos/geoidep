...
from app import db
from app.models import HerramientaGeografica
...
from app.services.instituciones import EXCLUDED_PARENT_IDS
...
WFS_FORMATOS_ARCGIS = (
  ('GEOJSON', 'GEOJSON'),
  ('GEOJSON+ZIP', 'GEOJSON+ZIP'),
  ('ESRIGEOJSON+ZIP', 'ESRIGEOJSON+ZIP'),
  ('KML', 'KML'),
  ('SHAPE+ZIP', 'SHAPE+ZIP'),
  ('CSV', 'CSV'),
...
def obtener_imagen_herramienta_url(herramienta_id):
  ...
  candidate_paths = []
  for directory in ('imagenes/herramientas_geograficas', 'imagenes'):
...
def obtener_configuracion_servicios():
  tipos = (
    Tipo.query.filter(
      Tipo.estado.is_(True),
      Tipo.id_padre.in_(DEFAULT_CAPA_PARENT_IDS),
    )
    .order_by(Tipo.orden.asc(), Tipo.nombre.asc())
    .all()
  )
...
def es_servicio_wfs_arcgis(base_url):
  ...
  # ArcGIS expone siempre su endpoint WFS con el segmento "WFSServer"
  # (tanto ArcGIS Server on-premise .../MapServer/WFSServer como los
  # hosted feature services de ArcGIS Online .../arcgis/services/<n>/WFSServer).
  if 'wfsserver' in path:
    return True

  # Respaldo por dominio: servicios alojados en ArcGIS Online.
  if host.endswith('.arcgis.com') or host == 'arcgis.com':
    return True
...
def obtener_sigla_servicio(tipo, id_servicio):
  if tipo is not None:
    sigla = getattr(tipo, 'sigla', None)
    if sigla:
...
def obtener_recurso_tipo(tipo, id_servicio):
  if tipo is not None:
    recurso = getattr(tipo, 'recurso', None)
    if recurso:
...
def obtener_tipos_catalogo():
  tipos = (
    Tipo.query.filter(
      Tipo.id_padre.in_((1, 2, 3)),
      Tipo.estado.is_(True),
    )
    .order_by(Tipo.orden.asc(), Tipo.nombre.asc())
    .all()
  ...
  if es_tipo_capa:
    query = (
      ...
      .filter(
        CapaGeografica.servicios.any(
          and_(
            ServicioGeografico.id_tipo == id_tipo,
            *(
    ...
    for capa in capas:
      ...
      for servicio in capa.servicios:
        if servicio.id_tipo not in configuracion_servicios['ids_capa']:
          continue
      ...
      def servicio_sort_key(item):
        ...
        if orden is None:
          orden = 9999
        sigla_orden = obtener_sigla_servicio(tipo, item.id_tipo) or ''
        return (
          orden,
          sigla_orden.lower(),
        )
      ...
      for servicio in sorted(servicios_relacionados, key=servicio_sort_key):
        ...
        recurso_tipo = obtener_recurso_tipo(
          servicio.tipo,
          servicio.id_tipo,
        )
        if recurso:
        ...
        if (
          estado_activo
          and sigla_display
          and 'WMS' in sigla_display.upper()
          and servicio.id_layer not in (None, 0, '0')
        ):
        ...
        if (
          estado_activo
          and not download_options
          and bool(getattr(servicio, 'descarga', False))
        ):
    ...
    categorias_disponibles_query = (
      ...
      .join(CapaGeografica.servicios)
      .filter(ServicioGeografico.id_tipo == id_tipo)
    )
    ...
    query = (
      HerramientaGeografica.query.options(
        joinedload(HerramientaGeografica.categoria),
        joinedload(HerramientaGeografica.institucion),
        joinedload(HerramientaGeografica.tipo),
      )
      .filter(HerramientaGeografica.id_tipo == id_tipo)
      .join(HerramientaGeografica.categoria)
    )

    if id_categoria:
      query = query.filter(HerramientaGeografica.id_categoria == id_categoria)

    if id_institucion:
      query = query.filter(HerramientaGeografica.id_institucion == id_institucion)

    if estado_filter is not None:
      query = query.filter(HerramientaGeografica.estado == bool(estado_filter))

    if filter_terms:
      pattern = f"%{filter_terms}%"
      query = query.filter(
        or_(
          ilike_unaccent(HerramientaGeografica.nombre, pattern),
          ilike_unaccent(HerramientaGeografica.descripcion, pattern),
        )
      )

    herramientas = query.order_by(
      func.lower(Categoria.nombre), func.lower(HerramientaGeografica.nombre)
    ).all()
      ...
      categoria_data['herramientas'].append(
        {
          ...
          'tipo': sanitize_text(
            herramienta.tipo.nombre if herramienta.tipo else None
          ),
          'servicios': [],
          'acciones': [],
          'es_capa': False,
    ...
    instituciones_disponibles = (
      Institucion.query.join(Institucion.herramientas_geograficas)
      .filter(
        HerramientaGeografica.id_tipo == id_tipo,
        ~Institucion.id_padre.in_(EXCLUDED_PARENT_IDS),
      )
    )
    ...
    if id_categoria:
      instituciones_disponibles = instituciones_disponibles.filter(
        HerramientaGeografica.id_categoria == id_categoria
      )

    if estado_filter is not None:
      instituciones_disponibles = instituciones_disponibles.filter(
        HerramientaGeografica.estado == bool(estado_filter)
      )
    ...
    categorias_disponibles_query = (
      Categoria.query.join(Categoria.herramientas_geograficas)
      .filter(HerramientaGeografica.id_tipo == id_tipo)
    )
...
def obtener_opciones_catalogo_globales():
  herramientas_exists = db.session.query(HerramientaGeografica.id).filter(
    HerramientaGeografica.id_institucion == Institucion.id
  ).exists()
    ...
    if selected_tipo['nombre']:
      ...
      for categoria in categorias_list:
        for herramienta in categoria.get('herramientas', []):
          if not herramienta.get('tipo'):
            herramienta['tipo'] = selected_tipo['nombre']
  else:
      ...
      for tipo in tipos_catalogo.lista:
        ...
        for categoria in datos_tipo['categorias']:
          ...
          for herramienta in categoria.get('herramientas', []):
            ...
            if not herramienta.get('tipo'):
              herramienta['tipo'] = tipo.get('nombre')
  ...
  def obtener_opciones_globales():
    ...
    if categorias_globales_cache is None or instituciones_globales_cache is None:
      categorias_globales_cache, instituciones_globales_cache = (
        obtener_opciones_catalogo_globales()
      )
    return categorias_globales_cache, instituciones_globales_cache

  if id_categoria:
    selected_categoria = next(
      (cat for cat in categorias_opciones if cat['id'] == id_categoria),
      None,
    )
    if not selected_categoria:
      ...
      if selected_categoria:
        categorias_opciones = sorted(
          [*categorias_opciones, selected_categoria],
          key=lambda item: (item['nombre'] or '').lower(),
        )
    if selected_categoria:
      applied_filters.append(
        {
          'label': selected_categoria['nombre'],
          'type': 'categoria',
        }
      )
  ...
  if id_institucion:
    ...
    if not selected_institucion:
      ...
      if selected_institucion:
        instituciones_opciones = sorted(
          [*instituciones_opciones, selected_institucion],
          key=lambda item: (item['nombre'] or '').lower(),
        )
    if selected_institucion:
...
@bp.route('/')
def principal():
  tipos_catalogo = obtener_tipos_catalogo()
  geoportales_count = HerramientaGeografica.query.filter(
      HerramientaGeografica.id_tipo == 5
  ).count()
  servicios_mapas_count = ServicioGeografico.query.count()
  visores_count = HerramientaGeografica.query.filter(
      HerramientaGeografica.id_tipo == 6
  ).count()
  catalogo_metadatos_count = HerramientaGeografica.query.filter(
      HerramientaGeografica.id_tipo == 9
  ).count()
