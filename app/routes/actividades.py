...
from app.services.auditoria import registrar_historico_actividad
...
def listar():
    ...
    instituciones = obtener_instituciones_para(
      usuario, excluir_sectores=True, excluir_padres=False, orden_por_sigla=True
    )
...
def actualizar(id_seg: int):
    ...
    registrar_historico_actividad(seg, 'UPDATE', usuario.id if usuario else None)
...
def eliminar(id_seg: int):
    ...
    registrar_historico_actividad(seg, 'DELETE', usuario.id if usuario else None)
