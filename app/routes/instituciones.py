...
@bp.route('/<int:id_institucion>', methods=['PUT'])
@jwt_required()
def actualizar(id_institucion: int):
  ...
  institucion.usuario_registra = usuario.id if usuario else None
  try:
    db.session.commit()
  except IntegrityError:
    db.session.rollback()
...
@bp.route('/<int:id_institucion>', methods=['DELETE'])
@jwt_required()
def eliminar(id_institucion: int):
  institucion = Institucion.query.get(id_institucion)
  if not institucion:
    return jsonify({'status': 'error', 'message': 'Institución no encontrada.'}), 404
  usuario = obtener_usuario_actual(requerido=True)
