...
from app import db
from app.config import URL_SERVIDOR_MAPAS
...
def preparar_url_capabilities(tipo, url):
  if not url:
    return url
...
def es_servicio_arcgis_mapserver(tipo):
  if not tipo:
    return False
  if tipo.id == 17:
    return True
...
def realizar_request_get(url, timeout=15, headers=None):
  ...
  try:
    ...
    # Muchos servidores OGC gubernamentales presentan certificados inválidos o
    # no reconocidos. Como solo se leen capabilities públicas de solo lectura,
    # se reintenta una vez sin verificación.
    logging.warning(
...
def inspeccionar_capabilities(contenido):
  """Lee (version, tag_local) de la raíz del documento Capabilities."""
  try:
    root = ET.fromstring(contenido)
  except ET.ParseError:
    return None, None
  tag = root.tag.split('}')[-1] if root.tag else None
  return root.get('version'), tag

def versiones_candidatas(tipo, contenido):
  """Versiones OWSLib a intentar, con la detectada en el documento primero."""
  if es_servicio_wms(tipo):
    soportadas = ['1.3.0', '1.1.1']
  elif es_servicio_wfs(tipo):
    soportadas = ['2.0.0', '1.1.0', '1.0.0']
  elif es_servicio_wmts(tipo):
    soportadas = ['1.0.0']
  else:
    return []

  version_doc, tag = inspeccionar_capabilities(contenido)
  preferida = None
  if es_servicio_wms(tipo):
    if tag == 'WMT_MS_Capabilities' or (version_doc or '').startswith(('1.1', '1.0')):
      preferida = '1.1.1'
    elif tag == 'WMS_Capabilities' or (version_doc or '').startswith('1.3'):
      preferida = '1.3.0'
  elif version_doc in soportadas:
    preferida = version_doc

  if preferida and preferida in soportadas:
    return [preferida] + [v for v in soportadas if v != preferida]
  return soportadas

def construir_capas_ogc(tipo, url, contenido):
  """Parsea el XML de capabilities con la versión que coincide con el documento.

  Se pasa ``xml=contenido`` para que OWSLib no realice su propia petición (que
  volvería a fallar por SSL) y para que el parser coincida con la versión que
  realmente devolvió el servidor.
  """
  if es_servicio_wms(tipo):
    cls = WebMapService
  elif es_servicio_wfs(tipo):
    cls = WebFeatureService
  elif es_servicio_wmts(tipo):
    cls = WebMapTileService
  else:
    raise ValueError('El tipo de servicio OGC seleccionado no está soportado.')

  ultimo_error = None
  for version in versiones_candidatas(tipo, contenido):
    try:
      servicio = cls(url, version=version, xml=contenido, headers=HEADERS_LEGALES)
...
def obtener_capas_desde_servicio(tipo, url):
  url_capabilities = preparar_url_capabilities(tipo, url)

  # Reconstruye la URL de GetCapabilities y, si no se obtienen capas, reintenta
  # forzando una versión explícita: algunos servidores (p.ej. GeoServer) sin el
  # parámetro version devuelven un documento que OWSLib no logra parsear.
  ...
  ultimo_error = None
  for url_intento in urls_intento:
    respuesta = validar_estado_servicio(url_intento)
    try:
      capas = construir_capas_ogc(tipo, url_intento, respuesta.content)
    except ValueError:
      raise
    except Exception as error:
      ultimo_error = error
  ...
  if ultimo_error is not None:
    ...
    if codigo_error:
      ...
      mensaje_error = (
        'No se pudo leer las capacidades del servicio. Es posible que la URL '
        'no exponga el documento Capabilities o que el origen bloquee la '
        'consulta (CORS / cabeceras restringidas).'
      )
...
def inicio():
  ...
  tipos_query = (
    Tipo.query.filter(Tipo.estado.is_(True))
    .filter(Tipo.id_padre.in_((2, 3)))
    .order_by(Tipo.id.asc(), Tipo.nombre.asc())
    .all()
  )
  tipos_serv = [
    {
  ...
  for capa in consulta.all():
    ...
    for servicio in capa.servicios:
      ...
      servicios_detalle.append(
        {
          'id': servicio.tipo.id,
          'nombre': servicio.tipo.nombre,
          'sigla': servicio.tipo.sigla,
    ...
    servicio_wms = next(
      (servicio for servicio in capa.servicios if servicio.id_tipo == 11),
      None,
    )
  ...
  servicios = [
    {
      ...
      'id_layer': servicio.id_layer,
      'estado': servicio.estado,
      'descarga': bool(servicio.descarga),
      'url_doc_descarga': servicio.url_doc_descarga,
      'tipo_nombre': servicio.tipo.nombre
      if servicio.tipo
      else None,
    }
    for servicio in capa.servicios
  ...
  return jsonify(
    {
      ...
      'descripcion': capa.descripcion,
      'tipo_capa': capa.tipo_capa,
      'id_layer_geoperu': id_layer_geoperu,
      'en_geoperu': bool(id_layer_geoperu),
      'id_categoria': capa.id_categoria,
      'id_institucion': capa.id_institucion,
      'servicios': servicios,
    }
...
def guardar():
  ...
  for servicio in servicios_payload:
    ...
    url_doc_descarga = (servicio.get('url_doc_descarga') or '').strip() or None
    estado_valor = servicio.get('estado', False)
    descarga_valor = servicio.get('descarga', False)
    valores_verdaderos = ('1', 'true', 't', 'si', 'sí', 'on', 'yes')
    if isinstance(estado_valor, str):
      estado_servicio = estado_valor.strip().lower() in valores_verdaderos
    else:
      estado_servicio = bool(estado_valor)
    if isinstance(descarga_valor, str):
      descarga_servicio = descarga_valor.strip().lower() in valores_verdaderos
    else:
      descarga_servicio = bool(descarga_valor)

    if not id_tipo or not direccion:
      return (
        jsonify(
          {
            'status': 'error',
    ...
    servicios_validos.append(
      {
        ...
        'id_layer': id_layer,
        'estado': estado_servicio,
        'descarga': descarga_servicio,
        'url_doc_descarga': url_doc_descarga,
        'visible': visible,
      }
    )

  tipos_consulta = (
    Tipo.query.filter(Tipo.id.in_(tipos_requeridos)).all()
    if tipos_requeridos
    else []
  )
  ...
  for servicio in servicios_validos:
    tipo = tipos_por_id.get(servicio['tipo_id'])
    if not tipo:
      return (
        jsonify(
          {
    ...
    requiere_unicidad = (
      tipo.estado
      and tipo.id_padre in (2, 3)
    )
    if requiere_unicidad:
      if tipo.id in tipos_controlados:
        return (
          jsonify(
            {
              'status': 'error',
    ...
    elif es_servicio_arcgis_mapserver(tipo):
      if servicio['estado']:
        if not servicio['nombre_capa'] or not servicio['titulo_capa']:
          return (
            jsonify(
    ...
    if tipo.id_padre in (2, 3) and not servicio['nombre_capa']:
      return (
        jsonify(
          {
            'status': 'error',
    ...
    if servicio['tipo_id'] == 11 and id_layer_geoperu is None:
      return (
        jsonify(
          {
            'status': 'error',
  ...
  for servicio in servicios_validos:
    ...
    servicio_instancia.id_tipo = servicio['tipo_id']
    ...
    servicio_instancia.descarga = servicio['descarga']
    servicio_instancia.url_doc_descarga = servicio['url_doc_descarga']
...
def obtener_capas_servicio():
  ...
  tipo = Tipo.query.get(tipo_id_int)
  if not tipo:
    return (
      jsonify(
        {
  ...
  try:
    if tipo.id_padre == 2:
      capas = obtener_capas_desde_servicio(tipo, url)
    elif es_servicio_arcgis_mapserver(tipo):
