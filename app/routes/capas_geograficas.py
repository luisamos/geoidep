...
from app.services.auditoria import (
  registrar_historico_capa,
  registrar_historico_servicio,
)
from app.services.database import sincronizar_secuencia
from app.services.instituciones import obtener_instituciones_para
from app.services.ogc import (
  IDS_PADRE_SERVICIOS,
  VALORES_TIPO_CAPA,
  es_servicio_arcgis_mapserver,
  obtener_capas_desde_mapserver,
  obtener_capas_desde_servicio,
)
from .helpers import obtener_usuario_actual, usuario_puede_editar, usuario_restringido_a_su_entidad
...
def inicio():
  ...
  tipos_query = (
    ...
    .filter(Tipo.id_padre.in_(IDS_PADRE_SERVICIOS))
  ...
  instituciones = obtener_instituciones_para(
    usuario, excluir_sectores=True, excluir_padres=False
  )
...
def guardar():
  ...
  if not usuario_puede_editar(usuario):
  ...
  if tipo_capa not in VALORES_TIPO_CAPA:
    return (
      jsonify(
        {
          'status': 'error',
