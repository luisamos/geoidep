...
from app.routes.helpers import obtener_usuario_actual, usuario_puede_editar, usuario_restringido_a_su_entidad
from app.services.auditoria import registrar_historico_herramienta
...
def inicio():
  ...
  tipos_servicio = (
    Tipo.query.filter_by(id_padre=1)
...
def guardar():
  ...
  id_institucion_payload = None
  if institucion_payload not in (None, ''):
    try:
      id_institucion_payload = int(institucion_payload)
    except (TypeError, ValueError):
      return (
        jsonify(
          {
  ...
  if usuario_restringido_a_su_entidad(usuario):
    ...
    id_institucion = id_institucion_payload or usuario.id_institucion
  ...
  sincronizar_secuencia(HerramientaGeografica)
  herramienta = HerramientaGeografica(
    nombre=datos['nombre'],
    descripcion=datos['descripcion'],
    estado=estado,
    recurso=datos['recurso'],
    id_tipo=datos['tipo'].id,
...
def actualizar(id):
    ...
    herramienta = HerramientaGeografica.query.get(id)
    if not herramienta:
    ...
    herramienta.id_tipo = datos['tipo'].id
    herramienta.id_categoria = datos['categoria'].id
    herramienta.usuario_registra = usuario.id
