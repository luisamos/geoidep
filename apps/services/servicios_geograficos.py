...
def build_getcapabilities_url(self, base_url: str, service: str) -> str:
  ...
  def build_service_url(self, servicio: ServicioGeografico) -> str:
    if servicio.id_tipo_servicio == self.WMS_SERVICE_TYPE:
      return self.build_getcapabilities_url(servicio.direccion_web, "WMS")
    if servicio.id_tipo_servicio == self.WFS_SERVICE_TYPE:
  ...
  async def procesar_servicio(
    ...
    if servicio.id_tipo_servicio in {
  ...
  def es_servicio_wms(self, tipo_servicio):
    if not tipo_servicio:
      return False
    if tipo_servicio.id == self.WMS_SERVICE_TYPE:
      return True
    nombre = (tipo_servicio.nombre or "").lower()
    return "wms" in nombre

  def es_servicio_wfs(self, tipo_servicio):
    if not tipo_servicio:
      return False
    if tipo_servicio.id == self.WFS_SERVICE_TYPE:
      return True
    nombre = (tipo_servicio.nombre or "").lower()
    return "wfs" in nombre

  def es_servicio_wmts(self, tipo_servicio):
    if not tipo_servicio:
      return False
  ...
  def es_servicio_arcgis_mapserver(self, tipo_servicio):
    if not tipo_servicio:
      return False
    if tipo_servicio.id == self.ARCGIS_REST_SERVICE_TYPE:
      return True
  ...
  def preparar_url_capabilities(self, tipo_servicio, url):
    if not url:
      return url
    if self.es_servicio_wms(tipo_servicio):
      return self.build_getcapabilities_url(url, "WMS")
    if self.es_servicio_wfs(tipo_servicio):
      return self.build_getcapabilities_url(url, "WFS")
    if self.es_servicio_wmts(tipo_servicio):
  ...
  def obtener_capas_desde_servicio(self, tipo_servicio, url):
    url_capabilities = self.preparar_url_capabilities(tipo_servicio, url)
    ...
    try:
      ...
      elif self.es_servicio_wfs(tipo_servicio):
        servicio = WebFeatureService(url_capabilities, headers=self.headers_legales)
        capas = self.construir_capas_desde_contenidos(servicio.contents)
      elif self.es_servicio_wmts(tipo_servicio):
